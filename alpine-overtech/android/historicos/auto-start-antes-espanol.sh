...
busybox chroot /data/local/linux/rootfs-jwm /bin/sh -c 'export PATH=/system/bin:/bin:/sbin:/usr/bin:/usr/sbin; export DISPLAY=:0; export HOME=/root; pcmanfm --desktop >/tmp/pcmanfm-desktop.log 2>&1 & exec jwm -f /root/.jwmrc' >> /data/local/linux/autostart.log 2>&1
